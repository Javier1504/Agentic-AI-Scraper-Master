...
from app.selector_prodi import pick_candidates_prodi
from app.extractors_prodi import SCHEMA_PRODI, RULES_PRODI, normalize_program_item
from app.io_prodi_excel import build_prodi_frame, save_prodi_outputs, load_prodi_template
DEFAULT_UNIV_XLSX = os.path.join(os.path.dirname(__file__), "IMPORT_FINAL.xlsx")
PRODI_TEMPLATE_XLSX = os.path.join(os.path.dirname(__file__), "(4) Import program studi.xlsx")
OUT_XLSX = os.path.join(OUT_DIR, "IMPORT_PRODI_FINAL.xlsx")
OUT_CSV  = os.path.join(OUT_DIR, "IMPORT_PRODI_FINAL.csv")
OUT_XLSX_PART = os.path.join(OUT_DIR, "IMPORT_PRODI_FINAL_partial.xlsx")
OUT_CSV_PART  = os.path.join(OUT_DIR, "IMPORT_PRODI_FINAL_partial.csv")
STATE_PATH = os.path.join(STATE_DIR, "state_run_prodi.json")
...
def _dedup_programs(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen = set()
    out = []
    for p in items:
        name = (p.get("name") or "").strip().lower()
        level = (p.get("level") or "-").strip().upper()
        faculty = (p.get("faculty") or "-").strip().lower()
        key = (name, level, faculty)
        if not name or name == "-":
            continue
        if key in seen:
            continue
...
def extract_multi_page(
    ...
    candidates = pick_candidates_prodi(base_url, r0.links or [], limit=limit_pages)
    ...
    for idx, url in enumerate(pages, start=1):
        ...
        data, usage = gem.extract_json(text=txt, schema=SCHEMA_PRODI, system_rules=RULES_PRODI)
    ...
    programs_all = _dedup_programs(programs_all)
...
def main():
    ...
    assert os.path.exists(PRODI_TEMPLATE_XLSX), f"Template prodi tidak ada: {PRODI_TEMPLATE_XLSX}"
    _ = load_prodi_template(PRODI_TEMPLATE_XLSX)

    assert os.path.exists(DEFAULT_UNIV_XLSX), (
        f"Input kampus tidak ada: {DEFAULT_UNIV_XLSX}\n"
        f"Taruh IMPORT_FINAL.xlsx di folder proyek (sejajar run_prodi_all.py)."
    )
    ...
    with PlaywrightFetcher() as fetcher:
        for i, r in univ.iterrows():
            ...
            try:
                ...
                if not programs:
                    print("[PRODI] hasil kosong jadi harus fallback gemini (URL-guided)", flush=True)
                    data2, usage2 = gem.extract_json_browse(
                        url=website, campus_name=name, schema=SCHEMA_PRODI, system_rules=RULES_PRODI
                    )
                    for k in total_usage:
                ...
                if not programs:
                    ...
                    print(f"[EMPTY] {name} | tidak menemukan prodi", flush=True)
                    continue
                ...
                for p in programs:
                    prodi_name = p["name"]
                    level = p.get("level", "-")
                    acc = p.get("accreditation", "-")
                    typ = json.dumps(p.get("type", ["Reguler"]), ensure_ascii=False)

                    faculty = p.get("faculty", "-")
                    desc = p.get("description", "-")
                    url_src = p.get("url", "-")
                    full_desc = f"Fakultas: {faculty}\nSumber: {url_src}\n\n{desc}".strip()

                    slug = slugify(f"{level}-{prodi_name}")

                    out_rows.append({
                        "id": next_id,
                        "major_id": None,
                        "university_id": _safe_int_or_raw(univ_id),
                        "name": prodi_name,
                        "slug": slug,
                ...
                if SAVE_EVERY_UNIV and ((i + 1) % SAVE_EVERY_UNIV == 0):
                    df_tmp = build_prodi_frame(rows)
                    save_prodi_outputs(df_tmp, OUT_XLSX_PART, OUT_CSV_PART)

                print(f"[DONE] {name} | programs={len(out_rows)} | total_tokens={total_usage['total_tokens']}")
    ...
    df_out = build_prodi_frame(rows)
    save_prodi_outputs(df_out, OUT_XLSX, OUT_CSV)
